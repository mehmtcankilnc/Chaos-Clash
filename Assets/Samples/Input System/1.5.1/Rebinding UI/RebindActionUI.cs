...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public void SaveNewBindings()
        {
            if (!ResolveActionAndBinding(out var action, out var bindingIndex))
                return;

            // Baðlama bilgilerini al
            var bindingId = action.bindings[bindingIndex].id.ToString();
            var binding = action.bindings[bindingIndex];

            // Baðlama bilgilerini özel bir sýnýfa dönüþtürerek JSON formatýna çevir
            var serializableBinding = new SerializableInputBinding(binding);
            var jsonBinding = JsonUtility.ToJson(serializableBinding);

            // PlayerPrefs'e kaydet
            PlayerPrefs.SetString(bindingId, jsonBinding);
            PlayerPrefs.Save();
        }

        // Özel bir sýnýf
        [System.Serializable]
        private class SerializableInputBinding
        {
            public string name;
            public string id;
            public string path;
